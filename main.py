...
def urls(current_user: User):
    if type(current_user) is User:
        return (Response(message=None, data={}, error=None)).parse(), 200
...
def sign_in():
    try:
        ...
        if token:
            return (Response(message=None, data={"token": token}, error=None)).parse(), 200
    except Exception as e:
        return (Response(message=str(e), data=None, error=str(e))).parse(), 500
...
def sign_up():
    try:
        if register_user(request.json, container.get_users_repository()) is True:
            return (Response(message=None, data=None, error=None)).parse(), 200
    except Exception as e:
        return (Response(message=str(e), data=None, error=str(e))).parse(), 500
